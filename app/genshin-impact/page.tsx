...
const genesisPackages = [
  { genesis: 60, price: 5.0 },
  { genesis: 300, price: 25.0 },
  { genesis: 980, price: 50.0 },
  { genesis: 1980, price: 100.0 },
  { genesis: 3280, price: 150.0 },
  { genesis: 6480, price: 300.0 },
]
...
export default function GenshinImpact() {
  ...
  const selectedPackageData = selectedPackage !== null ? genesisPackages[selectedPackage] : null
  const totalGenesis = selectedPackageData ? selectedPackageData.genesis * quantity : 0
  ...
  const handleProceedToCheckout = () => {
    ...
    const params = new URLSearchParams({
      ...
      currency: selectedPackageData.genesis.toString(),
      currencyType: "Genesis Crystals",
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="text-center lg:text-left">
              <h1 className="text-4xl md:text-5xl font-bold mb-2 text-white">Genshin Impact</h1>
              <p className="text-lg text-gray-300">Top-up your Genesis Crystals easily</p>
              <p className="text-sm text-gray-400 mt-2">Please double-check your UID and Server before proceeding.</p>
            </div>
            ...
            {/* Genesis Crystal Package Selection */}
            <Card className="bg-slate-800/50 border-slate-700 backdrop-blur-sm">
              <CardHeader>
                <CardTitle className="text-white">Select Genesis Crystal Package</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {genesisPackages.map((pkg, index) => (
                    <div
                      key={index}
                      onClick={() => setSelectedPackage(index)}
                      className={`p-4 rounded-lg border-2 cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-purple-500/20 ${
                    ...
                    >
                      <div className="text-center">
                        <div className="text-lg font-bold text-white">{pkg.genesis}</div>
                        <div className="text-sm text-gray-300">Genesis Crystals</div>
                  ...
                  {selectedPackageData ? (
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="text-white">{selectedPackageData.genesis} Genesis Crystals</span>
                      </div>
                      <div className="flex justify-between text-sm">
                      ...
                      <div className="flex justify-between font-semibold">
                        <span className="text-gray-300">Total Genesis Crystals:</span>
                        <span className="text-white">{totalGenesis}</span>
                      </div>
                    </div>
                  ) : (
