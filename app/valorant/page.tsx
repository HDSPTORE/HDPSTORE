...
const vpPackages = [
  ...
  { vp: 3250, price: 150.0 },
  { vp: 5350, price: 250.0 },
  { vp: 11000, price: 500.0 },
]

const regions = ["Asia Pacific", "North America", "Europe", "Korea", "Latin America", "Brazil"]
...
export default function Valorant() {
  ...
  const selectedPackageData = selectedPackage !== null ? vpPackages[selectedPackage] : null
  const totalVP = selectedPackageData ? selectedPackageData.vp * quantity : 0
  ...
  const handleProceedToCheckout = () => {
    if (!selectedPackageData || !riotId || !tagline || !selectedRegion || !selectedPayment) return

    const params = new URLSearchParams({
      game: "Valorant",
      userId: `${riotId}#${tagline}`,
      serverId: selectedRegion,
      currency: selectedPackageData.vp.toString(),
      currencyType: "Valorant Points",
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="text-center lg:text-left">
              <h1 className="text-4xl md:text-5xl font-bold mb-2 text-white">Valorant</h1>
              <p className="text-lg text-gray-300">Top-up your Valorant Points easily</p>
              <p className="text-sm text-gray-400 mt-2">
                Please double-check your Riot ID and Region before proceeding.
              </p>
            </div>
            ...
            <Card className="bg-slate-800/50 border-slate-700 backdrop-blur-sm">
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="riotId" className="text-gray-300">
                      Riot ID
                    </Label>
                    <Input
                      id="riotId"
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      placeholder="Enter your tagline (e.g., #1234)"
                      className="bg-slate-700/50 border-slate-600 text-white placeholder:text-gray-400 focus:border-white/30"
                    />
                  </div>
                </div>
            ...
            <Card className="bg-slate-800/50 border-slate-700 backdrop-blur-sm">
              <CardHeader>
                <CardTitle className="text-white">Select Valorant Points Package</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {vpPackages.map((pkg, index) => (
                    <div
                      ...
                      className={`p-4 rounded-lg border-2 cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-red-500/20 ${
                        selectedPackage === index
                          ? "border-red-400 bg-red-500/20 shadow-lg shadow-red-500/30"
                          : "border-slate-600 bg-slate-700/30 hover:border-red-400/50"
                      }`}
                    >
                      <div className="text-center">
                        <div className="text-lg font-bold text-white">{pkg.vp}</div>
                        <div className="text-sm text-gray-300">VP</div>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-300">Riot ID:</span>
                    <span className="text-white">{riotId && tagline ? `${riotId}#${tagline}` : "Not entered"}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-300">Region:</span>
                    <span className="text-white">{selectedRegion || "Not selected"}</span>
                  </div>
                </div>
                  ...
                  {selectedPackageData ? (
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="text-white">{selectedPackageData.vp} VP</span>
                      </div>
                      <div className="flex justify-between text-sm">
                      ...
                      <div className="flex justify-between font-semibold">
                        <span className="text-gray-300">Total VP:</span>
                        <span className="text-white">{totalVP}</span>
                      </div>
                    </div>
                  ) : (
                ...
                <div className="border-t border-slate-600 pt-4">
                  ...
                  <Button
                    ...
                    disabled={!selectedPackageData || !riotId || !tagline || !selectedRegion || !selectedPayment}
                  >
                    Proceed to Checkout
                  </Button>
                </div>
