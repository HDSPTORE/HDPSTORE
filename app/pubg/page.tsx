...
export default function PUBG() {
  ...
  const selectedPackageData = selectedPackage !== null ? ucPackages[selectedPackage] : null
  const totalUC = selectedPackageData ? selectedPackageData.uc * quantity : 0
  ...
  const handleProceedToCheckout = () => {
    ...
    const params = new URLSearchParams({
      ...
      userId,
      serverId,
      currency: selectedPackageData.uc.toString(),
      currencyType: "UC",
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="text-center lg:text-left">
              <h1 className="text-4xl md:text-5xl font-bold mb-2 text-white">PUBG Mobile</h1>
              <p className="text-lg text-gray-300">Top-up your UC (Unknown Cash) easily</p>
              <p className="text-sm text-gray-400 mt-2">
                Please double-check your User ID and Server ID before proceeding.
              </p>
            </div>
            ...
            <Card className="bg-slate-800/50 border-slate-700 backdrop-blur-sm">
              <CardHeader>
                <CardTitle className="text-white">Select UC Package</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {ucPackages.map((pkg, index) => (
                    <div
                      ...
                      className={`p-4 rounded-lg border-2 cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-blue-500/20 ${
                        selectedPackage === index
                          ? "border-blue-400 bg-blue-500/20 shadow-lg shadow-blue-500/30"
                          : "border-slate-600 bg-slate-700/30 hover:border-blue-400/50"
                      }`}
                    >
                      <div className="text-center">
                        <div className="text-lg font-bold text-white">{pkg.uc}</div>
                        <div className="text-sm text-gray-300">UC</div>
                  ...
                  {selectedPackageData ? (
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="text-white">{selectedPackageData.uc} UC</span>
                      </div>
                      <div className="flex justify-between text-sm">
                      ...
                      <div className="flex justify-between font-semibold">
                        <span className="text-gray-300">Total UC:</span>
                        <span className="text-white">{totalUC}</span>
                      </div>
                    </div>
                  ) : (
